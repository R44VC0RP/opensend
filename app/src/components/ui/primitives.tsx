...
export function DataTable<T>({ columns, rows, rowKey, onRowClick, selectedId, empty, loading = false, skeletonRows = 5, minRows = 0, tableRef }: { columns: Column<T>[]; rows: T[]; rowKey: (row: T) => string; onRowClick?: (row: T) => void; selectedId?: string; empty?: ReactNode; loading?: boolean; skeletonRows?: number; minRows?: number; tableRef?: (node: HTMLDivElement | null) => void }) {
  return <div ref={tableRef} className="ui-table-scroll" aria-busy={loading || undefined}>

...
export function SegmentsPage() {
  ...
  return <div className="audience-page"><PageHeader title="Segments" actions={<Button variant="primary" onClick={() => navigate('/segments/new')}><Plus size={16} />Create segment</Button>} /><div className="data-toolbar"><Input className="audience-search" aria-label="Search segments" placeholder="Search segments" value={search} onChange={event => { setSearch(event.target.value); setPage(1) }} /><span className="muted audience-count">{query.isPending ? <SkeletonText width={100} /> : query.data && `${number(query.data.total)} segments`}</span></div>

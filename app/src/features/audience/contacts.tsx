...
import { Alert, Button, Checkbox, ConfirmDialog, DataTable, Dialog, EmptyState, ErrorState, Field, Input, LoadingState, PageHeader, Pagination, SectionHeader, Select, StatusBadge, Tabs } from '../../components/ui'
...
export function ContactsPage() {
  ...
  return <div className="audience-page">
    ...
    <Tabs value={status} onValueChange={value => { setStatus(value); setPage(1) }} items={[{ value: '', label: 'All contacts', count: query.data?.facets?.all }, ...statusOptions.map(item => ({ ...item, count: query.data?.facets?.[item.value] }))]} />
    <div className="data-toolbar"><div className="cluster"><Input className="audience-search" aria-label="Search contacts" placeholder="Search email or name" value={search} onChange={event => { setSearch(event.target.value); setPage(1) }} /><Select aria-label="Filter by list" value={listId} onValueChange={value => { setListId(value); setPage(1) }} options={[{ value: '', label: 'All lists' }, ...(lists.data || []).map(list => ({ value: list.id, label: list.name }))]} /></div><span className="muted">{query.data && `${number(query.data.total)} contacts`}</span></div>
    {lists.isError && <ErrorState error={lists.error} onRetry={() => lists.refetch()} />}
    {query.isPending ? <LoadingState /> : query.isError ? <ErrorState error={query.error} onRetry={() => query.refetch()} /> : <><ContactTable contacts={query.data.items} lists={lists.data} /><Pagination page={page} pageSize={pageSize} total={query.data.total} onPageChange={setPage} /></>}
...
export function ContactDetailPage() {
  ...
  return query.isPending ? <LoadingState /> : query.isError ? <><PageHeader title="Contact" backTo="/contacts" /><ErrorState error={query.error} onRetry={() => query.refetch()} /></> : <ContactDetails key={id} contact={query.data} />
}
...
function ContactActivity({ email }: { email: string }) {
  ...
  return <section className="section"><SectionHeader title="Recent email activity" /><p className="muted">{regionId}</p>{query.isPending ? <LoadingState /> : query.isError ? <ErrorState error={query.error} onRetry={() => query.refetch()} /> : query.data.total ? <><DataTable rows={query.data.items} rowKey={row => row.id} columns={[{ key: 'subject', label: 'Email', render: row => <Link to={`/logs/${row.id}`}>{row.subject}</Link> }, { key: 'status', label: 'Latest event', render: row => <StatusBadge status={row.status} /> }, { key: 'time', label: 'Sent', render: row => date(row.sentAt) }]} /><Pagination page={page} pageSize={5} total={query.data.total} onPageChange={setPage} /></> : <EmptyState title="No email activity" description="No matching emails in the selected region." />}</section>
}

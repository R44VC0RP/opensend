...
export function RegionSetup() {
  ...
  return <>
    <section className="section stack">
      ...
      <DataTable loading={catalog.isPending} skeletonRows={2} rows={catalog.data?.data ?? []} rowKey={row => row.region} selectedId={entry?.region} empty={<EmptyState title="No regions configured" />} columns={[
  ...
  return <>
    ...
    <details className="settings-aws-details">
      ...
      <div className="stack">
        ...
        <DataTable rows={(['transactional', 'marketing'] as const).map(kind => ({kind, ...resources[kind]}))} rowKey={row => row.kind} columns={[

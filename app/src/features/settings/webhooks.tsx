...
export function WebhooksPage() {
  ...
  return <div className="stack">
    ...
    <SectionHeader title={webhooks.isPending ? <SkeletonText width={110} /> : 'Webhooks'} actions={<span className="muted">Workspace-wide · All regions</span>} />
...
function WebhookEditor({ webhook, regions }: { webhook: Webhook; regions: RegionCatalogEntry[] }) {
  ...
  const actionDescription = action === 'rotate' ? 'Only this endpoint’s signing secret will change. Copy the new secret once after rotation.' : action === 'delete' ? 'This endpoint and its delivery history will be removed. Other endpoints are unchanged.' : `${action === 'pause' ? 'Stop' : 'Resume'} deliveries to this endpoint across its scoped regions. Other endpoints are unchanged.`
  return <div className="stack">

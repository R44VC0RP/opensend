...
export function WebhooksPage() {
  ...
  return <div className="stack">
    ...
    <SectionHeader title={webhooks.isPending ? <SkeletonText width={110} /> : `${number(webhooks.data?.length ?? 0)} webhooks`} actions={<span className="muted">Workspace-wide · All regions</span>} />
    {webhooks.error ? <ErrorState error={webhooks.error} onRetry={() => void webhooks.refetch()} /> : <DataTable loading={webhooks.isPending} skeletonRows={3} minRows={3} rowSize="large" rows={webhooks.data ?? []} rowKey={webhook => webhook.id} onRowClick={webhook => navigate(`/settings/webhooks/${webhook.id}`)} empty={<EmptyState title="No webhook endpoints" action={<Button onClick={() => navigate('/settings/webhooks/new')}>Add webhook</Button>} />} columns={[
      ...
      { ...settingsColumns.webhooks[4], render: webhook => { const latest = [...webhook.deliveries].sort((a, b) => Date.parse(b.at) - Date.parse(a.at))[0]; return latest ? <div><span>{latest.response} · {label(latest.status)}</span><div className="muted">{date(latest.at)} · {time(latest.at)} UTC</div></div> : 'No deliveries' } },
    ]} />}
  </div>
}
...
function WebhookEditor({ webhook, regions }: { webhook: Webhook; regions: Region[] }) {
  ...
  return <div className="stack">
    ...
    <section className="section stack"><SectionHeader title="Delivery history" actions={<span className="muted">Time in UTC · {scopeText(webhook.regionIds)}</span>} /><MutationError error={retry.error} />
      <DataTable minRows={3} rowSize="large" rows={webhook.deliveries} rowKey={delivery => delivery.id} empty={<EmptyState title="No deliveries yet" />} columns={[

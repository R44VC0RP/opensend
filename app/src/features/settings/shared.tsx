...
export function SecretDialog({ secret, title, onClose }: { secret: string | null; title: string; onClose: () => void }) {
  return <Dialog open={secret !== null} onOpenChange={open => { if (!open) onClose() }} title={title} description="Copy this demo secret now. It will not be shown again after you close this dialog." footer={<Button variant="primary" onClick={onClose}>Done</Button>}>

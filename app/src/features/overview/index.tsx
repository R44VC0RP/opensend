...
function ActivityChart({ points, createdOnly = false }: { points: ChartPoint[]; createdOnly?: boolean }) {
  ...
  return <div ref={container} className="activity-chart" onMouseLeave={() => setActive(null)}><svg viewBox={`0 0 ${width} ${height}`} role="img" aria-label={createdOnly ? "Emails created per day on the left scale; outcomes for those emails on the right scale." : "Email activity. Sends use the left scale; bounces and complaints use the right scale."}>
    ...
    {tickIndices.map(i => <text key={i} x={x(i)} y={height - 13} textAnchor="middle" className="chart-label">{date(points[i].at, { month: 'short', day: 'numeric', timeZone: 'UTC' })}</text>)}
    {points.map((p, i) => {

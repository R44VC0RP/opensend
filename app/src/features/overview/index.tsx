...
export function OverviewPage() {
  ...
  return <><PageHeader title="Overview" actions={<div className="cluster"><span className="muted">{regionId}</span>{api.environment === 'test' ? <StatusBadge status="Test simulation" /> : current ? <StatusBadge status={current.access} tone={current.access === 'production' ? 'success' : 'warning'} /> : <Skeleton width={84} height={20} />}</div>} />{filterBar}
    {query.isPending ? <OverviewBodySkeleton /> : query.isError ? <ErrorState error={query.error} onRetry={() => query.refetch()} /> : data && <>
      ...
      {live && <p className="muted">Outcome counts apply to emails created in this period, not events occurring during it.</p>}
      <section className="chart-section" aria-label="Sending activity"><SectionHeader title={live ? "Emails created per day" : "Email activity"} actions={<div className="chart-legend"><span><i className="legend-sent" />{live ? 'Created' : 'Sent'}</span><span><i className="legend-bounces" />Bounces</span><span><i className="legend-complaints" />Complaints</span><span>UTC</span></div>} />{data.sent ? <ActivityChart points={data.points} createdOnly={live} /> : <EmptyState title="No emails in this period" description="Choose another period or email stream." />}</section>
      ...
      <section className="sending-streams"><SectionHeader title="Sending streams" />{data.streams.map(item => <div className="stream-row" key={item.name}><div className="cluster between"><span>{label(item.name)}</span><span>{number(item.sent)}</span></div><div className={`stream-track stream-track--${item.name}`}><span style={{ width: `${data.streams.reduce((sum, stream) => sum + stream.sent, 0) ? item.sent / data.streams.reduce((sum, stream) => sum + stream.sent, 0) * 100 : 0}%` }} /></div></div>)}<Button variant="ghost" size="sm" onClick={() => navigate('/logs')}>Explore email logs <ArrowUpRight size={16} /></Button></section></div>
    </>}
  </>
}
...
function ActivityChart({ points, createdOnly = false }: { points: ChartPoint[]; createdOnly?: boolean }) {
  ...
  return <div ref={container} className="activity-chart" onMouseLeave={() => setActive(null)}><svg viewBox={`0 0 ${width} 260`} role="img" aria-label={createdOnly ? "Emails created per day on the left scale; outcomes for those emails on the right scale." : "Email activity. Sends use the left scale; bounces and complaints use the right scale."}>
    ...
    {points.map((p, i) => {
      ...
      return <rect key={p.at} x={start} y={top} width={Math.max(0, end - start)} height={bottom - top} fill="transparent" tabIndex={0} role="button" aria-label={`${date(p.at)}: ${number(p.sent)} ${createdOnly ? 'created' : 'sent'}${hasOutcomes ? `, ${p.bounced} bounces, ${p.complaints} complaints` : ''}`} onMouseEnter={() => setActive(i)} onFocus={() => setActive(i)} onBlur={() => setActive(null)} />
    })}
    {selectedPoint && active !== null && <line x1={x(active)} x2={x(active)} y1={top} y2={bottom} className="chart-crosshair" />}
  </svg>{selectedPoint && <div className="chart-tooltip" role="status"><strong>{date(selectedPoint.at)}</strong><span>{createdOnly ? 'Created' : 'Sent'} <b>{number(selectedPoint.sent)}</b></span>{hasOutcomes && <><span>Bounces <b>{number(selectedPoint.bounced)}</b></span><span>Complaints <b>{number(selectedPoint.complaints)}</b></span></>}</div>}</div>
}

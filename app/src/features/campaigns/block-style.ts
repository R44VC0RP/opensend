...
const properties: Record<string, (value: string) => boolean> = {
  color, 'background-color': value => color(value) || value === 'transparent',
  ...
  padding: value => spacing(value, false), 'padding-top': value => spacing(value, false, 1), 'padding-right': value => spacing(value, false, 1), 'padding-bottom': value => spacing(value, false, 1), 'padding-left': value => spacing(value, false, 1),
  'border-radius': value => PX(0, 24).test(value) && parseFloat(value) <= 24, 'border-top-left-radius': value => PX(0, 24).test(value) && parseFloat(value) <= 24, 'border-top-right-radius': value => PX(0, 24).test(value) && parseFloat(value) <= 24, 'border-bottom-left-radius': value => PX(0, 24).test(value) && parseFloat(value) <= 24, 'border-bottom-right-radius': value => PX(0, 24).test(value) && parseFloat(value) <= 24,
  ...
  'border-color': color, 'border-top-color': color, 'border-right-color': color, 'border-bottom-color': color, 'border-left-color': color, width: dimension, height: dimension,
}
...
export function sanitizeBlockStyle(value: string | null | undefined): string | null {
  ...
  for (const declaration of declarations) { const separator = declaration.indexOf(':'); if (separator < 1) return null; const property = declaration.slice(0, separator).trim().toLowerCase(), candidate = declaration.slice(separator + 1).trim().toLowerCase(); if (!properties[property]?.(candidate) || /(?:url|var|calc|expression|!important|[{}\\])/.test(candidate)) return null; accepted.push([property, candidate]) }

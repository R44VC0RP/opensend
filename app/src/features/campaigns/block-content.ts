...
import { sanitizeBlockStyle } from './block-style'
...
const dimension = (value: unknown) => { const text = typeof value === 'number' && Number.isFinite(value) ? String(value) : typeof value === 'string' ? value.trim() : ''; return /^(?:[1-9][0-9]{0,3}|[1-9][0-9]?%|100%)$/.test(text) ? text : null }
...
const withStyle = (attrs: Record<string, unknown>, element: Element) => { const style = sanitizeBlockStyle(element.getAttribute('style')); return style ? { ...attrs, style } : attrs }
const blockAttrs = (attrs: Record<string, unknown>, element: Element) => withStyle(withAlignment(attrs, element), element)
...
function inlineNodes(parent: Node, marks: Mark[], inButton: boolean): EditorNode[] {
  ...
  for (const child of Array.from(parent.childNodes)) {
    ...
    else if (tag === 'SPAN') { const style = sanitizeBlockStyle(element.getAttribute('style')); if (style) add({ type: 'preservedStyle', attrs: { style } }) }
    else if (tag === 'A' && !inButton) {
      const href = safeHref(element.getAttribute('href'))
      if (href) { const target = element.getAttribute('target'), style = sanitizeBlockStyle(element.getAttribute('style')); add({ type: 'link', attrs: { href, target: target === '_blank' ? '_blank' : null, rel: target === '_blank' ? 'noopener noreferrer' : null, ...(style ? { style } : {}) } }) }
    }
    nodes.push(...inlineNodes(element, next, inButton))
  }
  return nodes
}
const paragraph = (element: Element, attrs: Record<string, unknown> = {}): EditorNode => ({ type: 'paragraph', attrs: blockAttrs(attrs, element), content: inlineNodes(element, [], false) })
...
function listNode(element: Element): EditorNode {
  ...
  for (const child of Array.from(element.children)) {
    ...
    items.push({ type: 'listItem', attrs: withStyle({}, child), content: [text.content?.length ? text : emptyParagraph(), ...nested] })
  }
  const start = Number(element.getAttribute('start'))
  return { type: ordered ? 'orderedList' : 'bulletList', attrs: withStyle(ordered && start > 1 ? { start } : {}, element), content: items.length ? items : [{ type: 'listItem', content: [emptyParagraph()] }] }
}
function imageNode(element: Element, link?: Element): EditorNode | null {
  ...
  return { type: 'image', attrs: withStyle({ src, alt: element.getAttribute('alt') ?? '', width: width ?? 'auto', height: height ?? 'auto', alignment: alignmentOf(element) ?? 'left', href: link ? safeHref(link.getAttribute('href')) : null }, element) }
}
function blockNodes(parent: Node, inColumn: boolean): EditorNode[] {
  ...
  for (const child of Array.from(parent.childNodes)) {
    ...
    const element = child as Element
    const tag = element.tagName
    if (tag in HEADING_LEVELS) { flush(); nodes.push({ type: 'heading', attrs: blockAttrs({ level: HEADING_LEVELS[tag] }, element), content: inlineNodes(element, [], false) }); continue }
    if (tag === 'P') {
      flush()
    ...
    if (tag === 'BLOCKQUOTE') {
      ...
      nodes.push({ type: 'blockquote', attrs: withStyle({}, element), content: inner.length ? inner : [paragraph(element)] })
      continue
    }
    if (tag === 'PRE') { flush(); const text = element.textContent ?? ''; nodes.push({ type: 'codeBlock', attrs: withStyle({ language: null }, element), content: text ? [{ type: 'text', text }] : [] }); continue }
    if (tag === 'HR') { flush(); nodes.push({ type: 'horizontalRule', attrs: withStyle({}, element) }); continue }
    ...
    if (tag === 'A') {
      ...
      if (isButton && href) { flush(); nodes.push({ type: 'button', attrs: withStyle({ href, alignment: alignmentOf(element) ?? 'left' }, element), content: inlineNodes(element, [], true).map(node => node.type === 'text' ? { ...node, marks: node.marks?.filter(mark => mark.type === 'bold') } : node) }); continue }
    ...
    if (tag === 'DIV' && element.hasAttribute('data-columns') && !inColumn) {
      ...
      if (type) { nodes.push({ type, content: columns.map(column => { const inner = blockNodes(column, true); return { type: 'columnsColumn', attrs: withStyle({}, column), content: inner.length ? inner : [emptyParagraph()] } }) }); continue }
    }
    if (tag === 'SECTION') { flush(); const inner = blockNodes(element, false); nodes.push({ type: 'section', attrs: withStyle({}, element), content: inner.length ? inner : [emptyParagraph()] }); continue }
...
const styleAttr = (attrs: Record<string, unknown> | undefined) => { const style = sanitizeBlockStyle(typeof attrs?.style === 'string' ? attrs.style : null); return style ? ` style="${escape(style)}"` : '' }
const MARK_TAGS: Record<string, string> = { bold: 'strong', italic: 'em', underline: 'u', strike: 's', code: 'code', sup: 'sup' }
const MARK_ORDER = ['link', 'bold', 'italic', 'underline', 'strike', 'code', 'sup', 'uppercase', 'preservedStyle']
...
function exportInline(node: EditorNode, inButton: boolean): string {
  ...
  for (const mark of marks) {
    if (mark.type === 'link' && !inButton) {
      ...
      if (href) html = `<a href="${escape(href)}"${mark.attrs?.target === '_blank' ? ' target="_blank"' : ''}${styleAttr(mark.attrs)}>${html}</a>`
    } else if (mark.type === 'uppercase') html = `<span style="text-transform:uppercase">${html}</span>`
    else if (mark.type === 'preservedStyle') { const style = sanitizeBlockStyle(typeof mark.attrs?.style === 'string' ? mark.attrs.style : null); if (style) html = `<span style="${escape(style)}">${html}</span>` }
...
function exportList(node: EditorNode): string {
  ...
  const items = (node.content ?? []).filter(item => item.type === 'listItem').map(item => {
    ...
    return `<li${styleAttr(item.attrs)}>${parts.join('')}</li>`
  })
  return `<${tag}${tag === 'ol' && start > 1 ? ` start="${start}"` : ''}${styleAttr(node.attrs)}>${items.join('')}</${tag}>`
}
function exportBlock(node: EditorNode, inColumn: boolean): string {
  switch (node.type) {
    case 'paragraph': return `<p${alignAttr(node.attrs)}${styleAttr(node.attrs)}>${exportInlineContent(node)}</p>`
    case 'heading': { const level = Math.min(3, Math.max(1, Number(node.attrs?.level) || 1)); return `<h${level}${alignAttr(node.attrs)}${styleAttr(node.attrs)}>${exportInlineContent(node)}</h${level}>` }
    case 'bulletList': case 'orderedList': return exportList(node)
    case 'blockquote': return `<blockquote${styleAttr(node.attrs)}>${(node.content ?? []).map(child => child.type === 'paragraph' ? `<p${alignAttr(child.attrs)}${styleAttr(child.attrs)}>${exportInlineContent(child)}</p>` : exportBlock(child, inColumn)).join('')}</blockquote>`
    case 'codeBlock': return `<pre${styleAttr(node.attrs)}><code>${escape((node.content ?? []).map(child => child.text ?? '').join(''))}</code></pre>`
    case 'horizontalRule': return `<hr${styleAttr(node.attrs)}>`
    ...
    case 'image': {
      ...
      const image = `<img src="${escape(src)}" alt="${escape(typeof node.attrs?.alt === 'string' ? node.attrs.alt : '')}"${width ? ` width="${width}"` : ''}${height ? ` height="${height}"` : ''}${alignAttr(node.attrs)}${styleAttr(node.attrs)}>`
      const href = safeHref(typeof node.attrs?.href === 'string' ? node.attrs.href : null)
      return href ? `<a href="${escape(href)}">${image}</a>` : image
    }
    case 'button': {
      const href = safeHref(typeof node.attrs?.href === 'string' ? node.attrs.href : null)
      return href ? `<a data-button href="${escape(href)}"${alignAttr(node.attrs)}${styleAttr(node.attrs)}>${exportInlineContent(node, true)}</a>` : `<p${alignAttr(node.attrs)}${styleAttr(node.attrs)}>${exportInlineContent(node)}</p>`
    }
    case 'twoColumns': case 'threeColumns': case 'fourColumns': {
      ...
      return `<div data-columns="${columns.length}">${columns.map(column => `<div data-column${styleAttr(column.attrs)}>${trimTrailingEmpty(column.content ?? []).map(child => exportBlock(child, true)).join('') || '<p></p>'}</div>`).join('')}</div>`
    }
    case 'section': return `<section${styleAttr(node.attrs)}>${trimTrailingEmpty(node.content ?? []).map(child => exportBlock(child, false)).join('') || '<p></p>'}</section>`
...
export function documentToBlockHtml(doc: EditorNode): string {
  ...
  const unwrap = (nodes: EditorNode[]): EditorNode[] => nodes.flatMap(node => ['container', 'body', 'div'].includes(node.type) ? unwrap(node.content ?? []) : node.type === 'globalContent' || node.type === 'previewText' ? [] : [node])

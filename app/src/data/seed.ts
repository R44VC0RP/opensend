...
export function createSeed(now = Date.now()): DemoState {
  ...
  const campaigns: Campaign[] = [
    ...
    return { ...value, status, subject: value.name, previewText: 'The latest news and updates from Acme.', fromName: 'Acme', fromEmail: value.regionId === 'eu-west-1' ? 'hello@acme.eu' : 'hello@acme.com', replyTo: [], html: `<h1>${value.name}</h1><p>Here is what is new at Acme.</p><p>This is a demo campaign. No email will be sent.</p>`, createdAt: ago((15 + i) * 24), updatedAt: ago((i + 1) * 8), scheduledAt: null, timezone: 'UTC', recipients: deliveredEmails.length, delivered: deliveredEmails.filter(e => e.status === 'delivered').length, bounced: deliveredEmails.filter(e => e.status === 'bounced').length, complaints: deliveredEmails.filter(e => e.status === 'complaint').length }
  })
  const templateDraft = { name: 'Product announcement', description: 'A reusable product-news layout.', subject: 'What’s new at Acme', previewText: 'A quick look at the latest release.', fromName: 'Acme', replyTo: [], html: '<h1>Product announcement</h1><p>Share what changed and why it matters.</p><p><a href="https://example.com" data-button="true">Learn more</a></p>', attachments: [] }

...
export interface Campaign { revision?: number; reviewId?: string | null; draft?: Record<string, any>; attachments?: string[]; id: string; regionId: RegionId; name: string; subject: string; previewText: string; fromName: string; fromEmail: string; listId: string; segmentId: string | null; html: string; editor?: CampaignEditorMetadata | null; status: CampaignStatus; createdAt: ISODate; updatedAt: ISODate; scheduledAt: ISODate | null; timezone: string; recipients: number; delivered: number; bounced: number; complaints: number }
export type CampaignInput = Pick<Campaign, 'regionId' | 'name' | 'subject' | 'previewText' | 'fromName' | 'fromEmail' | 'listId' | 'segmentId' | 'html' | 'editor'> & { id?: string; revision?: number; draft?: Record<string, any>; attachments?: string[] }
...
export interface ApiKey { environment?: 'live' | 'test'; revokedAt?: string | null; id: string; name: string; prefix: string; permission: 'send' | 'read'; domainId: string | null; createdAt: ISODate; lastUsedAt: ISODate | null }
export interface ApiKeyInput { environment?: 'live' | 'test'; name: string; permission: 'send' | 'read'; domainId: string | null }
...
export interface Domain { dnsStatus?: 'available' | 'unavailable'; dnsUnavailableReason?: string | null; id: string; regionId: RegionId; name: string; status: 'verified' | 'pending' | 'issue'; mailFromStatus: 'verified' | 'pending' | 'not_configured' | 'failed'; records: DnsRecord[]; createdAt: ISODate }
...
export interface OpenSendApi {
  ...
  campaigns: { list(input: PageRequest, signal?: AbortSignal): Promise<PageResult<Campaign>>; get(id: string, signal?: AbortSignal): Promise<Campaign>; save(input: CampaignInput, signal?: AbortSignal): Promise<Campaign>; audience(input: { listId: string; segmentId: string | null }, signal?: AbortSignal): Promise<AudiencePreview>; send(input: SendCampaignInput, signal?: AbortSignal): Promise<Campaign | CampaignSendReceipt>; test(input: { id: string; to: string }, signal?: AbortSignal): Promise<{ accepted: boolean; id?: string; status?: string; simulated?: boolean }> }
  ...
  keys: { list(signal?: AbortSignal, cursor?: string): Promise<CursorItems<ApiKey>>; create(input: ApiKeyInput, signal?: AbortSignal): Promise<CreatedApiKey>; revoke(id: string, signal?: AbortSignal): Promise<void> }

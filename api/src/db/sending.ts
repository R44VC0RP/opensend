...
export const attachmentLinks = pgTable('sending_attachment_links', {
  ...scope(), attachmentId: text('attachment_id').notNull().references(() => attachments.id), ownerType: text('owner_type').$type<'email' | 'campaign'>().notNull(), ownerId: text('owner_id').notNull(),

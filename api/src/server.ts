...
try {
  ...
  app.use('*', async (c, next) => {
    ...
    if (!['GET', 'HEAD'].includes(c.req.method) || /^\/(?:v1|api|mcp|authoring|template-assets|\.well-known|unsubscribe)(?:\/|$)/.test(c.req.path) || ['/health', '/openapi.json'].includes(c.req.path)) return next();

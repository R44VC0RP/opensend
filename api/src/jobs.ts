...
export async function processJobs(runtime: Runtime, handlers: Record<string, JobHandler>, limit = 1, concurrency = 1, maxDurationMs = 20000) {
  const started = Date.now(); let processed = 0, claims = 0, activeJobs = 0, peakActiveJobs = 0, activeSends = 0, peakActiveSends = 0;
  ...
  while (claims < Math.min(Math.max(1, limit), 100) && Date.now() - started < budgetMs) {
    claims++;
  ...
  try {
    ...
    log('info', { code: 'JOB_DRAIN', workspaceId: runtime.config.workspaceId, processed, claims, concurrency: effectiveConcurrency, peakActiveJobs, peakActiveSends, laneJobs, budgetMs, durationMs: Date.now() - started });
  }
}

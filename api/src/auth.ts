...
export const authenticate: MiddlewareHandler<AppEnv> = async (c, next) => {
  ...
  if (delegated) {
    ...
    if (token.startsWith('os_agent_')) {
      ...
      if (!temporary || temporary.v === 2 && (!tracked || tracked.revokedAt || Date.parse(tracked.expiresAt) !== temporary.exp || tracked.grantId !== temporary.grant || tracked.environment !== temporary.environment || JSON.stringify(tracked.permissions) !== JSON.stringify(temporary.permissions) || JSON.stringify(tracked.domains) !== JSON.stringify(temporary.domains)) || !grant || (temporary.environment === 'live' && grant.environment !== 'live') || temporary.permissions.some(permission => !grant.permissions.includes('manage') && !grant.permissions.includes(permission))) throw new ApiError(401, 'AUTH_INVALID', 'The temporary API token is invalid, expired, or its originating approval was revoked.');

/* eslint-disable */
// Generated by Wrangler by running `wrangler types --env-file=/dev/null --include-runtime=false` (hash: 22bb8e44ad0a5834c14169715a77c60a)
interface __BaseEnv_Env {
	ATTACHMENTS: R2Bucket;
	HYPERDRIVE: Hyperdrive;
	WAKE_QUEUE: Queue;
	FEEDBACK_QUEUE: Queue;
	ADMISSION: RateLimit;
	LOADER: WorkerLoader;
	BROWSER: BrowserRun;
	PUBLIC_URL: "https://opensend.anoma.ly";
	DEFAULT_SES_REGION: "us-east-2";
	ENABLE_LIVE_SES: "true";
	JOB_CONCURRENCY: "6";
	SIMULATED_SES_LATENCY_MS: "170";
	SIMULATED_SES_RATE: "5000";
	DISPATCH_SHARDS: "4";
	DISPATCH_RATE_FACTOR: "1.1";
	WEBHOOK_ALLOWED_HOSTS: "";
	DISPATCHER: DurableObjectNamespace<import("./src/worker").DispatcherShard>;
}
declare namespace Cloudflare {
	interface GlobalProps {
		mainModule: typeof import("./src/worker");
		durableNamespaces: "DispatcherShard";
	}
	interface Env extends __BaseEnv_Env {}
}
interface Env extends __BaseEnv_Env {}
type StringifyValues<EnvType extends Record<string, unknown>> = {
	[Binding in keyof EnvType]: EnvType[Binding] extends string ? EnvType[Binding] : string;
};
declare namespace NodeJS {
	interface ProcessEnv extends StringifyValues<Pick<Cloudflare.Env, "PUBLIC_URL" | "DEFAULT_SES_REGION" | "ENABLE_LIVE_SES" | "JOB_CONCURRENCY" | "SIMULATED_SES_LATENCY_MS" | "SIMULATED_SES_RATE" | "DISPATCH_SHARDS" | "DISPATCH_RATE_FACTOR" | "WEBHOOK_ALLOWED_HOSTS">> {}
}

...
One Hono/TypeScript API for Node/Docker and Cloudflare Workers. The dashboard and generated SDK use that same public API; MCP discovers its OpenAPI contract. PostgreSQL is the source of truth: queued mail rows are the dispatch queue, a Postgres outbox drives campaign orchestration and webhooks, and private R2/S3 stores attachments. On Cloudflare a Durable Object per environment/region hosts the long-lived dispatcher and a queue carries verified provider feedback; no Redis, D1, passwords, teams, or production admin bypass.

## Large campaigns

The normal send/schedule request needs only the current `revision`: when `reviewId` is omitted, OpenSend snapshots and validates the audience durably, then begins delivery automatically. The `202` response means the complete background workflow was accepted, not that validation or SES delivery has finished. Callers may instead pass a ready review created through the background review endpoints.
...
| Preparation/expansion chunk | Up to 20,000 / 2,000 recipients respectively; preparation is also bounded to 4 MiB and ten seconds |
| Active campaign dispatch buffer | Ten seconds of the cached regional send rate per campaign (400 floor, 20,000 cap) and twenty seconds per environment (1,000 floor, 50,000 cap); full buffers are rechecked within two seconds |
| Outstanding campaign intents | 1,000,000 per originating credential; 2,000,000 per environment |
| Orchestration jobs per process/invocation | `JOB_CONCURRENCY`: default 2, maximum 8 for Node and 6 for Cloudflare (campaign preparation/expansion/completion, webhooks, maintenance; never mail) |
| Dispatcher shards | `DISPATCH_SHARDS` Durable Objects per environment/region on Cloudflare (default 4), each pacing 1/N of the regional quota with up to six provider calls in flight (about 35 sends/s per shard at 170 ms provider latency); the Node runner dispatches in-process |

These are application bounds, not a guaranteed SES send rate. Mail is dispatched by long-lived batch dispatchers (`src/dispatcher.ts`): one statement leases up to twenty due rows, one transaction elects the batch (origin authorization, consent locks, campaign cancellation, unsubscribe tokens, attempt events), provider calls run six-wide behind an in-memory pacing gate, and one statement records every outcome. `sending_region_limits` is the shared one-minute quota cache; only a shard that finds it stale calls `GetAccount`. No SES request holds a database transaction, and a row is sent at most once because the claim flips it `queued → attempting` under its dispatch version. Direct sends retain their separate pending limits. Node runners hold one in-process gate each, so run one runner per installation or split the quota explicitly.

Apply migrations through `019_batch_dispatcher.sql` before deploying this backend; pre-migration `email.dispatch` job rows complete as no-ops once the dispatcher owns queued rows. For Docker, stop old runners and upgrade API/runner images together before accepting background preparations; old runners do not recognize the new job types. Do not roll back to an older backend while new-format reviews or campaigns are active. Legacy synchronous review IDs are not accepted for new delivery; omit `reviewId` to prepare the current revision automatically.
...
`wrangler.jsonc` contains the production bindings for **https://opensend.anoma.ly**. To self-host in another account, replace `account_id`, the custom-domain route and `PUBLIC_URL`, the Hyperdrive ID, and the R2/Queue names with your own resources; create both queues (`wrangler queues create opensend-work` and `wrangler queues create opensend-feedback`) before the first deploy. The `DispatcherShard` Durable Object migration is applied by `wrangler deploy`; `DISPATCH_SHARDS` sets how many shards share each region's quota. Do not reuse production bindings for staging or preview builds. The dashboard build is served from `app/dist` as Workers assets on the API's origin.
...
HTTP, Queue, and scheduled handlers share the outbox. Queue messages are wakeups; committed jobs survive a lost wakeup. Workers consume one wake signal per queue invocation and stop claiming new jobs after a two-second turn, while allowing every in-flight job to finish. Database claims lease only as many jobs as there are free local lanes; lanes refill individually as jobs finish, without waiting for the slowest job in a group. At most one feedback-first reservation is active per invocation, with feedback-only fallback when no send work exists. Invocation-local randomized 16-turn rotation approximates live/test and job-class fairness without writing a shared scheduler row on every claim. The `job_schedule` row remains for admission locking and wake coalescing. Campaign expansion emits a bounded burst of up to eight wake signals after committing new email jobs. Cron recovers due work and performs hourly retention. Scheduling prefers live/test work 3:1 with fallback. PostgreSQL transactions/leases, not wake messages or process-local state, control dispatch.
...
## Isolated sending benchmarks

**Test-environment** mail always uses the full dispatch path and the real SES SDK with an in-process transport instead of AWS networking. Live mail never selects this transport. Defaults: `SIMULATED_SES_LATENCY_MS=100`, `SIMULATED_SES_RATE=1000`, and `SIMULATED_SES_DELIVERY_DELAY_MS=250`. Override the latency when modeling a different observed provider call; zero latency measures application overhead only.

Simulated dispatch verifies attachment bytes, constructs MIME, paces against the cached test-region quota, claims consent/authorization, and stores acceptance atomically. It then emits two synthetic Send/Delivery callbacks that substitute for incoming SNS requests: on Cloudflare they travel the same feedback queue as verified live callbacks; on Node and in the acceptance fixture they are durable `operation.simulatedFeedback` jobs. Both paths run the same routine-ingestion statement, creating receipts when they arrive. Statuses progress through accepted/sent/delivered, but all records remain `simulated: true` and provider IDs start with `sim_`; they are not evidence of actual delivery. The public SNS endpoint still requires real signature/account verification. AWS-stored templates are explicitly rejected in test mode; template discovery/rendering and AWS-side validation are not emulated. Simulated feedback uses the same receipt, status, and fan-out logic, but does not certify SNS signature verification or real mailbox delivery.

`api/scripts/benchmark-sending.ts` requires a dedicated local `opensend_perf_*` database, all migrations, explicit `ENABLE_LIVE_SES=false`, and no AWS credentials. Run via `node --import tsx scripts/benchmark-sending.ts` with that isolated configuration. `BENCHMARK_RECIPIENTS` is capped at 1,000; `BENCHMARK_REPLICAS`, `BENCHMARK_LANES`, and `BENCHMARK_CONNECTIONS` control local load. Optional `BENCHMARK_REPORT` saves JSON metrics. Fixtures remain in the isolated database for inspection. Do not run it concurrently with an independent job runner. It measures the shared Node/database pipeline, not Cloudflare Queue startup or Hyperdrive latency; short high-rate runs do not establish sustained capacity.

Verified SNS callbacks never touch the sending jobs table. On Cloudflare the `/v1/events/ses` handler checks the SNS signature and account binding, enqueues the callback on the `opensend-feedback` queue and returns 202; a separate consumer ingests batches of up to 100 within two seconds. Routine Delivery (and any remaining Send) callbacks are persisted by one set-based statement per environment: receipt, matched event, monotonic status (the highest-ranked new event wins when several share a batch), public event and matching webhook deliveries. Bounce, complaint, engagement and subscription callbacks are processed inline with their receipt, and messages are acknowledged or retried individually. If a callback arrives before provider acceptance is recorded, its receipt and a deferred recovery job commit together. The configuration set no longer subscribes to `SEND` events—acceptance is recorded from the `SendEmail` response—so re-run provisioning after upgrading to update the event destination. The admission rate limiter does not apply to this endpoint. On Node the handler ingests routine callbacks inline and queues non-routine ones as jobs. With no matching subscribers, no webhook job is created.

## SES and production release gates

Leave `ENABLE_LIVE_SES=false` until real sending is deliberately configured. It prevents live mail, **not read-only AWS discovery or explicitly confirmed provisioning**. Test keys run the same dispatch pipeline through the in-process SES transport without AWS calls, production suppression effects, or production engagement; they do not certify real MIME rendering/delivery.
...
| Campaigns | Versioned drafts/reviews, previews, scheduling, and cancellation before dispatch. Draft `html` is block HTML (`GET /v1/campaign-content-guide`); the server validates it on save and renders the styled email plus a plain-text part at review/test/send, so the dashboard composer, SDK and MCP share one content document. `/v1/campaigns/{id}/reviews` supports up to 1,000,000 matching contacts in durable, bounded batches; poll its returned review ID until `ready`, or omit `reviewId` when sending to run preparation automatically. CSV imports remain capped at 1,000 rows per import. Larger input fails rather than truncates. Revocation cannot recall accepted or in-flight mail. |
...
| Retries / rate | In-memory pacing per dispatcher shard from the cached regional quota. Provider throttling requeues the row with 2s, 4s, 8s, 16s, 32s backoff (five attempts); transient preflight failures retry after 15s; ambiguous provider acceptance is never automatically replayed. Orchestration jobs keep six bounded attempts delayed 15s, 1m, 4m, 16m, and 1h and remain inspectable. |

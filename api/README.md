...
HTTP, Queue, and scheduled handlers share the outbox. Queue messages are wakeups; committed jobs survive a lost wakeup. Each Queue invocation claims one local concurrency-sized job group and returns after every in-flight job finishes, allowing Queues to reassess horizontal concurrency after each short batch. A database claim leases at most one immediately runnable job per local lane instead of serializing one scheduler update per lane. One slot prioritizes authenticated SES feedback; the remaining slots prefer dispatch/control work and fall back to feedback/publication work when no such job is runnable. Campaign expansion emits one wake signal per local claim group, capped at 100, after committing new email jobs. Cron recovers due work with longer bounded drains and performs hourly retention. Scheduling prefers live/test work 3:1 with fallback. PostgreSQL transactions/leases, not wake messages or process-local state, control dispatch.

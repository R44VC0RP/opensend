...
`SIMULATED_SES_ENABLED=true` opts **test-environment** mail into the full dispatch path. It uses the real SES SDK with an in-process transport instead of AWS networking. Live mail never selects this transport. Defaults: `SIMULATED_SES_LATENCY_MS=200`, `SIMULATED_SES_RATE=1000`, and `SIMULATED_SES_DELIVERY_DELAY_MS=250`. Use 300–350 ms latency to approximate the observed live provider calls; zero latency measures application overhead only.

Simulated dispatch verifies attachment bytes, constructs MIME, reserves the shared test-region quota, claims consent/authorization, and stores acceptance atomically with two durable synthetic callback jobs. Those jobs substitute for incoming SNS requests and run the same routine-ingestion statement as verified live callbacks, creating receipts when they arrive. Statuses may progress through accepted/sent/delivered, but all records remain `simulated: true` and provider IDs start with `sim_`; they are not evidence of actual delivery. The public SNS endpoint still requires real signature/account verification. Existing test behavior remains unchanged when the option is off. AWS-stored templates are explicitly rejected in latency-simulator mode; template discovery/rendering and AWS-side validation are not emulated. Simulated send/delivery feedback uses the same receipt, status, and fan-out logic, but does not certify SNS signature verification or real mailbox delivery.

`api/scripts/benchmark-sending.ts` requires a dedicated local `opensend_perf_*` database, all migrations, explicit `ENABLE_LIVE_SES=false`, no AWS credentials, and the simulator enabled. Run via `node --import tsx scripts/benchmark-sending.ts` with that isolated configuration. `BENCHMARK_RECIPIENTS` is capped at 1,000; `BENCHMARK_REPLICAS`, `BENCHMARK_LANES`, and `BENCHMARK_CONNECTIONS` control local load. Optional `BENCHMARK_REPORT` saves JSON metrics. Fixtures remain in the isolated database for inspection. Do not run it concurrently with an independent job runner. It measures the shared Node/database pipeline, not Cloudflare Queue startup or Hyperdrive latency; short high-rate runs do not establish sustained capacity.

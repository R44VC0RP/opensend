...
Lifecycle events and matching outbound webhook deliveries are persisted with the email-state update; no intermediary publication job is needed. With no matching subscribers, no webhook job is created. Audience publication batches retain their bulk path. Feedback still performs durable database work and must be included when reporting benchmark completion.

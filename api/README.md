...
Simulated dispatch verifies attachment bytes, constructs MIME, reserves the shared test-region quota, claims consent/authorization, and stores acceptance atomically with two durable synthetic callback jobs. Those jobs substitute for incoming SNS requests and run the same routine-ingestion statement as verified live callbacks, creating receipts when they arrive. Statuses progress through accepted/sent/delivered, but all records remain `simulated: true` and provider IDs start with `sim_`; they are not evidence of actual delivery. The public SNS endpoint still requires real signature/account verification. AWS-stored templates are explicitly rejected in test mode; template discovery/rendering and AWS-side validation are not emulated. Simulated send/delivery feedback uses the same receipt, status, and fan-out logic, but does not certify SNS signature verification or real mailbox delivery.
...
Verified SNS Send/Delivery callbacks persist their receipt, monotonic email status, public event, and matching outbound webhook deliveries in one database statement in the HTTP Worker. Routine callbacks do not consume sending-job lanes or trigger wakeups without downstream work. If a callback arrives before provider acceptance is mapped to an email, its receipt and a deferred recovery job commit together. Non-routine feedback retains the durable safety-processing path. With no matching subscribers, no webhook job is created. Audience publication batches retain their bulk path. Synthetic callback vehicle jobs add scheduling overhead not present in real SNS HTTP ingress; report this distinction when comparing benchmark completion.
